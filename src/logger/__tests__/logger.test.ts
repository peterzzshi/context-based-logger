...
const mockWrite = jest.fn<(str: string) => boolean>();
const originalWrite = process.stdout.write;

const getLogOutput = () => {
  const call = mockWrite.mock.calls[0];
  if (!call?.[0]) throw new Error('No log output captured');
...
describe('Logger', () => {
  it('should log basic messages', () => {
    ...
    expect((logOutput.details as Record<string, unknown>).timestamp).toBeDefined();
  });
  ...
  it('should log errors with stack traces', () => {
    ...
    expect((logOutput.details as Record<string, unknown>).stack).toBeDefined();
  });
  ...
  it('should include context in logs', () => {
    ...
    const logOutput = getLogOutput();
    const details = logOutput.details as Record<string, unknown>;

    expect(logOutput.sessionId).toBe('test-session');
    expect(details.category).toBe('test');
    expect(details.tags).toEqual(['test-tag']);
